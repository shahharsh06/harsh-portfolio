...
describe("Projects Component", () => {
  ...
  it("displays project technologies", () => {
    ...
    const technologyTags = screen.getAllByText(
      /React|TypeScript|Tailwind|Node.js|Python/,
    );
  ...
  it("handles carousel navigation for other projects", async () => {
    ...
    const otherProjectNavButtons = otherProjectButtons.filter(
      (button) =>
  ...
  it("pauses/resumes on hover via handlers (combined)", () => {
    render(<Projects />);
    const container =
      (document.querySelector(".flex.overflow-hidden") as HTMLElement) ||
  ...
  it("handles section icon interactions", async () => {
    ...
    const sectionIcons = document.querySelectorAll(
  ...
  it("handles external link accessibility", () => {
    ...
    const demoLinks = externalLinks.filter(
      (link) =>
        link.getAttribute("href")?.includes("demo") ||
        link.getAttribute("href")?.includes("live"),
    );
  ...
  it("handles carousel infinite scrolling", async () => {
    ...
    const nextButtons = screen
      .getAllByRole("button")
      .filter((button) => button.getAttribute("aria-label")?.includes("next"));
  ...
  it("handles carousel navigation with keyboard", async () => {
    ...
    const carouselButtons = navigationButtons.filter(
      (button) =>
        button.getAttribute("aria-label")?.includes("Previous") ||
  ...
  it("navigation buttons work at boundaries", async () => {
    ...
    const nextButtons = screen
      .getAllByRole("button")
      .filter((btn) => btn.getAttribute("aria-label")?.includes("next"));
    const prevButtons = screen
      .getAllByRole("button")
      .filter((btn) => btn.getAttribute("aria-label")?.includes("previous"));
