...
describe("Navigation internal functions", () => {
  ...
  it("scrollToSection does nothing if element does not exist", () => {
    ...
    expect(() =>
      scrollToSection("#notfound", mockSetIsMobileMenuOpen),
    ).not.toThrow();
    expect(mockSetIsMobileMenuOpen).toHaveBeenCalledWith(false);
  });
  ...
  it("SocialIcons renders all provided social links", () => {
    const socialLinks = [
      ...
      },
      {
        ...
        icon: () => React.createElement("span", null, "LI"),
      },
      {
        label: "Dashboard",
        href: "/harsh-portfolio/dashboard.html",
        icon: () => React.createElement("span", null, "DB"),
      },
    ];
    const { getByText } = render(
      React.createElement(SocialIcons, { socialLinks }),
    );
